...
import { toggleDiagramReaction } from "../actions";
import type { ReactionSummary } from "@/lib/wiki";

const PALETTE = ["👍", "❤️", "🎉", "🚀", "👀", "😄", "🙏", "🔥"];

export default function DiagramReactions({ diagramId, reactions }: { diagramId: string; reactions: ReactionSummary[] }) {
  ...
  async function toggle(emoji: string) {
    if (busy) return;
    setBusy(true);
    await toggleDiagramReaction(diagramId, emoji);
    setBusy(false);
    setOpen(false);
    router.refresh();
  }
    ...
    <div className="flex flex-wrap items-center gap-1.5">
      {reactions.map((r) => (
        <button
          ...
          className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-sm transition-colors ${r.mine ? "border-blue-300 bg-blue-50" : "border-neutral-200 hover:bg-neutral-50"}`}
        >
          <span>{r.emoji}</span>
