...
import { addDiagramComment, deleteDiagramComment } from "../actions";
import type { CommentItem } from "@/lib/wiki";
import { CommentComposer, CommentContent } from "../../wiki/comment-editor";
...
export default function DiagramComments({ diagramId, comments, currentUserId }: { diagramId: string; comments: CommentItem[]; currentUserId: string }) {
  ...
  async function post(parentId: string | null, body: string, done: () => void) {
    if (busy) return;
    setBusy(true);
    const r = await addDiagramComment(diagramId, parentId, body);
    setBusy(false);
